...
import { INITIAL_BUILDING_STATUS, type BuildingStatus } from './buildings';
...
export default function Home() {
  ...
  const [buildingStatus,setBuildingStatus]=useState<BuildingStatus>(INITIAL_BUILDING_STATUS);
  ...
  useEffect(()=>{
    ...
    import('./terrain-scene').then(async({MountainScene})=>{
      ...
      try{
        const scene=new MountainScene(container.current,{onPeak:p=>{void selectRef.current(p).catch(()=>{});},onInteraction:()=>setTour(false),onPosition:(lng,lat,bearing)=>setPosition({lng,lat,bearing}),onError:setError,onFeature:feature=>{setGeoFeature(feature);if(feature.layer==='rivers'&&stepRef.current!=='explore'){setStep('scenario');setFloodRiver(feature);}if(feature.kind==='residential'&&floodContext.current.result)setStep('review');setGeoOpen(true);setTour(false);},onGeography:setGeoStatus,onBuildings:setBuildingStatus,onWindow:bounds=>setTerrainBounds(bounds),onContinuing:setContinuing});
  ...
  useEffect(()=>{mountain.current?.setBuildings(buildingsVisible);},[buildingsVisible,ready]);
  ...
  return <main className="atlas">
    ...
    <div className={`workspace ${geoOpen?'explorer-open':''} ${floodResult?'has-assessment':''} unified-workspace`}>
      ...
      <section className="map-stage" aria-label="Interactive 3D terrain of Nepal">
        ...
        {floodResult&&<div className="flood-map-legend" aria-label="Flood exposure map legend"><strong>{floodResult?`Scenario +${floodResult.scenario.stage.toFixed(1)} m`:floodRiver?.name||'Choose a river reach'}</strong><span><i className="wet"/>Estimated inundated land</span><span><House size={14} className="exposed"/>Residential portion exposed</span>{(!floodResult||floodResult.showSensitivity)&&<span><i className="sensitive"/>+1 m sensitivity fringe</span>}{floodResult?.baseline&&<span><i className="comparison"/>Comparison +{floodResult.baseline.stage.toFixed(1)} m</span>}<span><i className="unassessed"/>Outside assessment</span><small>Residential land assessment · Individual buildings unassessed</small></div>}
        <div hidden={!!floodResult||(!geoLayers.settlements&&!buildingsVisible)} className="settlement-legend" aria-label="Settlement map legend">{geoLayers.settlements&&<><House size={14} aria-hidden="true"/><span>Place labels</span><i aria-hidden="true"/><span>Residential land</span></>}{buildingsVisible&&<><i className="building-swatch" aria-hidden="true"/><span>{buildingStatus.loading?'Buildings loading…':buildingStatus.error?'Building coverage incomplete':buildingStatus.mode==='footprints'?'Footprints + surrounding density':'Building density'}</span></>}</div>
